...
//   // Listeners = everyone not currently a speaker. You always appear first.
//   const listeners = participants
//     .filter((p) => !speakerIds.has(p.session_id))
//     .sort((a, b) => {
//       if (a.session_id === mySessionId) return -1
//       if (b.session_id === mySessionId) return 1
//       return 0
//     })
...
//       {/* Scrollable middle section: speaker grid + divider + listener grid */}
...
//                     <div
//                       key={p.session_id}
//                       className={`bg-white/[0.06] rounded-lg flex flex-col items-center justify-center py-2.5 px-1 relative transition-all ${
//                         activeSpeaker === p.session_id ? 'ring-[1.5px] ring-purple-400' : ''
//                       } ${p.session_id === mySessionId ? 'border border-white/25' : ''}`}
//                     >
//                       {p.photoUrl ? (
//                         <img src={p.photoUrl} alt={p.user_name} className="w-8 h-8 rounded-full object-cover" />
//                       ) : (
//                         <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-[10px] font-medium">
//                           {p.user_name.slice(0, 2).toUpperCase()}
//                         </div>
//                       )}
//                       <span className="text-white text-[11px] mt-1 max-w-full truncate">
//                         {p.user_name}
//                       </span>
//                       {!p.audio ? (
//                         <MicOff size={10} className="absolute top-1.5 right-1.5 text-white/40" />
//                       ) : (
//                         <Mic
//                           size={10}
//                           className={`absolute top-1.5 right-1.5 ${
//                             activeSpeaker === p.session_id ? 'text-purple-400' : 'text-white/40'
//                           }`}
//                         />
//                       )}
//                     </div>
...
//       {/* Fixed bottom: reactions + controls, never scroll away */}
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      call.on('joined-meeting', () => {
        hasJoinedOnce = true
        setJoined(true)
        // Mic on by default for everyone, per current design
        ...
        // Start real audio-level monitoring for simultaneous multi-speaker glow
        try {
          call.startLocalAudioLevelObserver(500)
          call.startRemoteParticipantsAudioLevelObserver(500)
        } catch (e) {
          // Non-critical if unsupported — glow simply won't activate
        }
      })
      ...
      // Full rebuild only when membership actually changes (someone joins)
      call.on('participant-joined', () => {
        updateParticipants()
      ...
      // Targeted update — patch only the one participant that changed,
      // instead of rebuilding the whole list on every single event.
      call.on('participant-updated', (ev: any) => {
        const p = ev.participant
        if (!p) return

        setParticipants((prev) => {
          const exists = prev.some((item) => item.session_id === p.session_id)
          if (!exists) {
          ...
          return prev.map((item) =>
            item.session_id === p.session_id
              ? { ...item, audio: p.audio, user_name: p.user_name || item.user_name }
              : item
          )
        })

        if (p.local) setMyMuted(!p.audio)
      })
      ...
      // Real audio-level detection — supports multiple simultaneous glowing tiles
      call.on('local-audio-level', (ev: any) => {
        const localId = call.participants().local?.session_id
        if (!localId) return
        setSpeakingIds((prev) => {
          const next = new Set(prev)
          if (ev.audioLevel > SPEAKING_THRESHOLD) next.add(localId)
      ...
      call.on('remote-participants-audio-level', (ev: any) => {
        setSpeakingIds((prev) => {
          const next = new Set(prev)
          Object.entries(ev.participantsAudioLevel || {}).forEach(([sid, level]: any) => {
            if (level > SPEAKING_THRESHOLD) next.add(sid)
    ...
    return () => {
      try {
        callRef.current?.stopLocalAudioLevelObserver()
        callRef.current?.stopRemoteParticipantsAudioLevelObserver()
      } catch (e) {}
      Object.values(audioElementsRef.current).forEach((el) => el.remove())
      audioElementsRef.current = {}
      wakeLockRef.current?.release?.()
          ...
          <>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {speakers.map((p) => (
                <div
                  key={p.session_id}
                  className={`bg-white/10 rounded-xl aspect-square flex flex-col items-center justify-center relative transition-all ${
                    p.audio && speakingIds.has(p.session_id) ? 'ring-2 ring-[#C9A227]' : ''
                  }`}
                >
                  {p.photoUrl ? (
                    <img src={p.photoUrl} alt={p.user_name} className="w-14 h-14 rounded-full object-cover" />
            ...
            {listeners.length > 0 && (
              <>
                ...
                <div className="grid grid-cols-4 gap-2 pb-4">
                  {listeners.map((p) => {
                    const isSpeakingNow = p.audio && speakingIds.has(p.session_id)
                    return (
                      <div
                        key={p.session_id}
                        className={`bg-white/[0.06] rounded-lg flex flex-col items-center justify-center py-2.5 px-1 relative transition-all ${
                          isSpeakingNow ? 'ring-[1.5px] ring-purple-400' : ''
                        } ${p.session_id === mySessionId ? 'border border-white/25' : ''}`}
                      >
                        {p.photoUrl ? (
                          <img src={p.photoUrl} alt={p.user_name} className="w-8 h-8 rounded-full object-cover" />
                        ...
                        {!p.audio ? (
                          ...
                          <Mic
                            size={10}
                            className={`absolute top-1.5 right-1.5 ${
                              isSpeakingNow ? 'text-purple-400' : 'text-white/40'
                            }`}
                          />
                        )}
                      </div>
                ...
                return (
                  <div key={p.session_id} className="flex items-center gap-3">
                    ...
                    {p.audio ? (
                      <Mic
                        size={14}
                        className={isSpeakingNow ? 'text-[#C9A227]' : 'text-gray-400'}
                      />
                    ) : (
                      <MicOff size={14} className="text-gray-400" />
                    )}
